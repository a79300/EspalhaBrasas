...
async def fetch_valorant_stats(username: str, tag: str, mode: str, api_key: str) -> Dict[str, Any]:
    """
...
def create_stats_embed(data: Dict[str, Any], mode: str) -> discord.Embed:
    ...
    if lifetime_data.get('data') and lifetime_data['data']:
        for match in lifetime_data['data']:
            ...
            match_damage = stats.get('damage', {}).get('made', 0) if isinstance(stats.get('damage'), dict) else 0
    ...
    avg_damage = round(total_dmg_per_round / matches_played) if matches_played > 0 else 0
    
    # Agente mais jogado
    most_played_agent = max(agent_count, key=agent_count.get) if agent_count else "Desconhecido"
    agent_icon = get_agent_icon(most_played_agent)
    agent_games = agent_count.get(most_played_agent, 0) if most_played_agent != "Desconhecido" else 0
    ...
    if matches_data.get('data') and matches_data['data']:
        ...
        if match_history:
            embed.add_field(name="🎯 Histórico de Partidas", value=match_history, inline=False)
